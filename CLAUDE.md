...
9. **PWA / service worker.** El SW (`public/sw.js`) solo intercepta GETs same-origin: nunca la API, Cloudinary, HTML ni RSC. Si tocas `public/offline.html` (o cualquier ruta de `PRECACHE_URLS`), **bumpea `SW_VERSION`** en el mismo commit: sin eso los clientes instalados siguen sirviendo la copia vieja para siempre, porque el único disparador de update es el byte-diff de `sw.js`. Emergencia: `public/sw.kill.js` lleva el procedimiento en su cabecera. Para probarlo hace falta build de producción (`npm run start`, config `dots-webapp-prod` del launch.json) — en dev el registro se desactiva a propósito.
   ...
   **La invitación a instalar** es cosa aparte del SW: `lib/install-prompt.ts` decide y redacta (puro, con tests), `lib/install-browser.ts` toca `window` y guarda la marca, `hooks/use-install-prompt.ts` mira el navegador y `components/pwa/` pinta. `beforeinstallprompt` se captura en el layout **raíz** (`install-capture.tsx`) porque llega una sola vez y pronto: engancharlo en el hub llega tarde. El aviso salta al volver de terminar una actividad, solo en móvil y **dos veces como mucho** por dispositivo; la marca vive en `localStorage` y no en `users.settings` porque instalar es un acto de ESE dispositivo. iOS no tiene API: ahí solo hay tutorial, y dentro de Instagram o Facebook el tutorial correcto es «ábreme en Safari». El camino de Android tampoco se puede probar en dev, por el mismo motivo que el SW.
