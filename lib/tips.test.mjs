...
test("las tres rutas restantes también llegan a su pista", () => {
  // Sin esto, una errata en la cadena de la ruta compilaría y pasaría todos
  // los tests: la pista simplemente no saldría nunca, y solo se notaría
  // probando la app a mano.
  assert.deepEqual(pendientesPara("/play", []).map((t) => t.key), ["arcade.diarios"]);
  assert.deepEqual(pendientesPara("/review", []).map((t) => t.key), ["repaso.que-es"]);
  assert.deepEqual(pendientesPara("/quests", []).map((t) => t.key), ["retos.torneo"]);
});

test("toda pista del catálogo es alcanzable por su propia ruta", () => {
  // Cierra el hueco de raíz: recorre el catálogo en vez de enumerar rutas.
  for (const t of TIPS) {
    const claves = pendientesPara(t.ruta, []).map((x) => x.key);
    assert.ok(claves.includes(t.key), `${t.key} no sale en ${t.ruta}`);
  }
});
