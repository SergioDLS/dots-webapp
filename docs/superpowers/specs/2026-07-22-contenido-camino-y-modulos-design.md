...
#### F3e — Dominio por ítem + sesiones por tramos + autoplay + ronda inversa — ✅ hecho y verificado 2026-07-24
**Estado de ejecución:** backend (`5d0e08f`) y frontend (`87a862f`) implementados según el [plan](../plans/2026-07-22-f3e-dominio-tramos.md); migración `migrate:item-progress --apply` corrida por el usuario 2026-07-24 (backup `scripts/out/backup-item-progress-1784920551776.json`).
**✅ Verificado en preview (2026-07-24, sesión real contra prod):** tramos (letras piden 7 de 26, decenas 4 de 8, days 7/7 completo), autoplay al cambiar de turno, ronda inversa funcionando, `PUT /path/nodes/:id/progress` responde **200** (fin del 400 de letters/numbers) con `{nodeProgress, mastery, xpGained}`, dominio por ítem persistente (una letra fallada volvió priorizada con `timesWrong:1`), **dos niveles** visibles (abecedario 26%/23%, days 100% con "👑 Dominado 0%", pronunciación 100%/87%), y corona renderizando en el camino (nodo de lectura completado; en módulos exige `mastery >= 100`, aún sin pack dominado al 100%).
