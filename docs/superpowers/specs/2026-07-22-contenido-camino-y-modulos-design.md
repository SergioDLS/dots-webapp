...
#### F3e — Dominio por ítem + sesiones por tramos + autoplay + ronda inversa — 🔨 código implementado 2026-07-22 (falta: aplicar migración + preview)
**Estado de ejecución:** backend (`5d0e08f`) y frontend (`87a862f`) implementados y con build verde según el [plan](../plans/2026-07-22-f3e-dominio-tramos.md). **Pendiente:** `npm run migrate:item-progress -- --apply` en `dots-backend` (bloqueado para el agente por ser write a prod — lo corre el usuario; dry-run verificado OK) y la verificación en preview (sin la tabla, las lecciones letters/numbers/vocab del backend nuevo fallan al cargar).
